...
func (i *Test) launchDaemon(coreBinaryPath string) {
	...
	config.HcnetCoreURL = "http://localhost:" + strconv.Itoa(hcnetCorePort)
	config.CoreRequestTimeout = time.Second * 2
	config.HcnetCoreBinaryPath = coreBinaryPath
...
func findDockerComposePath() string {
	...
	if gopath := os.Getenv("GOPATH"); gopath != "" {
		monorepo := filepath.Join(gopath, "src", "github.com", "hcnet", "soroban-tools")
