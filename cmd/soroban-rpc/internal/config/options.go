...
func (cfg *Config) options() ConfigOptions {
	...
	defaultHcnetCoreBinaryPath, _ := exec.LookPath("hcnet-core")
	cfg.optionsCache = &ConfigOptions{
		...
		{
			Name:      "hcnet-core-url",
			Usage:     "URL used to query Hcnet Core (local captive core by default)",
		...
		{
			Name:         "hcnet-core-timeout",
			Usage:        "Timeout used when submitting requests to hcnet-core",
			ConfigKey:    &cfg.CoreRequestTimeout,
			DefaultValue: 2 * time.Second,
		},
		{
			Name:         "hcnet-captive-core-http-port",
		...
		},
		{
			...
			Usage:        "path to hcnet core binary",
		...
		},
		{
			...
			Usage:     "path to additional configuration for the Hcnet Core configuration file used by captive core. It must, at least, include enough details to define a quorum set",
		...
		},
		{
			...
			Usage:     "Network passphrase of the Hcnet network transactions should be signed for. Commonly used values are \"" + network.FutureNetworkPassphrase + "\", \"" + network.TestNetworkPassphrase + "\" and \"" + network.PublicNetworkPassphrase + "\"",
